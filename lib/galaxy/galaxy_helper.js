...
var GalaxyHelper = function() {
	...
	self.getRandomSeed = function(star) {
		return (star != null) ? (star.id + self.config._randomSeed) : self.config._randomSeed;
	};
	...
	self.generateProceduralStarTable = function(aabb, options) {
		...
		for (var i=0; i<self.config.starCount; i++) {
			...
			star.x = self.randomRangeInt(aabb.x.min, aabb.x.max);
			star.y = self.randomRangeInt(aabb.y.min, aabb.y.max);
			star.z = 0;
	...
	self.generateGalaxy = function(starArray, options) {
		...
		data2.aabb = self.generateProceduralBoundingBox(options);
		console.log("generateProceduralBoundingBox took " + (new Date().getTime() - start) + "ms");

		start = new Date().getTime();
		data2.stars = self.generateProceduralStarTable(1000, data2.aabb, options);
		console.log("generateProceduralBoundingBox took " + (new Date().getTime() - start) + "ms");
		console.log("generateProceduralBoundingBox data2=" + JSON.stringify(data2));
		//return data2;
		*/
