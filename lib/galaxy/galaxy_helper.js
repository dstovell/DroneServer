...
var GalaxyHelper = function() {
	...
	self.generateProceduralStarTable = function(aabb, spirals, options) {
		...
		if (useOld) {
			...
			var baseChance = Math.pow(self.config.starCount, 1.5) / ((aabb.x.max-aabb.x.min) * (aabb.y.max-aabb.y.min));
			...
			for (var x=aabb.x.min; x<aabb.x.max; x++) {
				for (var y=aabb.y.min; y<aabb.y.max; y++) {
					...
					var maxSpiralDist = 5.0;
					var spiralDist = 0.0;//((spirals != null) && (spirals.length > 0)) ? self.getDistanceFromAnySpiral({x:x, y:y}, spirals) : 0;
					var spiralChance = (maxSpiralDist - spiralDist) / maxSpiralDist;
					spiralChance = Math.max(spiralChance, 0.1);
					//console.log("spiralDist=" + spiralDist + " spiralChance=" + spiralChance);
					
					var chance = baseChance*posChance*posChance*spiralChance*spiralChance;
					var chanceInv = Math.max(1-chance, 0.0);
					var diceRoll = self.randomRangeFloat(0, 1);
					//console.log("posRadius=" + posRadius);
					//console.log("posChance=" + posChance);
					//console.log("chanceInv=" + chanceInv);
					//console.log("diceRoll=" + diceRoll);
					//console.log("====================");
					if (diceRoll > chanceInv) {
