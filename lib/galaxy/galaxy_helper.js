		...
		if (useOld) {
			...
			for (var x=aabb.x.min; x<aabb.x.max; x++) {
				for (var y=aabb.y.min; y<aabb.y.max; y++) {
					...
					else if (self.config.generateMode == "spiral") {
						var maxSpiralDist = 5.0;
						var spiralDist = maxSpiralDist;//((spirals != null) && (spirals.length > 0)) ? self.getDistanceFromAnySpiral({x:x, y:y}, spirals) : 0;
						spiralChance = 1 + 10000 * (maxSpiralDist - spiralDist);
						spiralChance = Math.max(spiralChance, 1.0);
					}
					
					//console.log("spiralDist=" + spiralDist + " spiralChance=" + spiralChance);
					
					var chance = baseChance*posChance*spiralChance;
