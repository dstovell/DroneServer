...
function StarMapController(options) {
	...
	self.config = {
		starmap: {
			...
			starTypes: [
				{name:'yellow', color:'yellow', radius:10},
				{name:'red', color:'red', radius:10},
				{name:'blue', color:'blue', radius:10}
			]
		}
	}
...
StarMapController.prototype.generateNewMap = function(options, cb) {
	...
	var cornerA = self.vector2(0, 0);
	var cornerB = self.vector2(width, height);
